  ...
  1   i=3-itsst
       cff=time+0.5D0*dt
        ...
        if (ierr .ne. 0) goto 7
        ierr=nf_get_var1_double (ncidfrc, sst_tid, sst_rec, cff)
        if (ierr .ne. nf_noerr) then
          write(stdout,6) 'sst_time', sst_rec
          goto 99
        endif
        sst_time(i)=cff*day2sec+sst_cycle*sst_ncycle
        if (sst_time(itsst).eq.-1.D+20) sst_time(itsst)=sst_time(i)
        if (lsstgrd.eq.1) then
          ierr=nf_fread (sstg(0,0,i), ncidfrc, sst_id,
     &                                           sst_rec, r2dvar)
        else
          ierr=nf_get_var1_double (ncidfrc, sst_id, sst_rec, sstp(i))
        endif
        if (ierr .ne. nf_noerr) then
        ...
        if (lsstgrd.eq.1) then
          ierr=nf_fread (dqdtg(0,0,i), ncidfrc, dqdt_id,
     &                                            sst_rec,  r2dvar)
        else
          ierr=nf_get_var1_double (ncidfrc,dqdt_id,sst_rec,dqdtp(i))
        endif
        if (ierr .ne. nf_noerr) then
